...
public class CartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int num = 1;
            if (cart.getItemById(id) != null) {
                num = cart.getQuantityOfItemById(id) + 1;
            } else {
                num = 1;
            }
        ...
        if (returnUrl_raw != null) {
            ...
            if (returnUrl.contains(".jsp")) {
                ...
                if (url.contains("productdetail")) {
                    response.sendRedirect(url + "?productID=" + id_raw);
                } else {
                    response.sendRedirect(url);
                }
            } else {
                if (returnUrl.contains("productdetail")) {
                    response.sendRedirect(returnUrl + "?productID=" + id_raw);
                } else {
