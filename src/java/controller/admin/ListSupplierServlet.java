 ...
 * @author duykh
 */
@WebServlet(name = "ListSupplierServlet", urlPatterns = {"/listsupplier"})
public class ListSupplierServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<Supplier> list = sdb.getListSupplier(3);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            switch (status) {
                case "1": {
                    List<Supplier> list = sdb.getListSupplier(1);
                    request.setAttribute("listSupplier", list);
                    request.setAttribute("status", status);
                    break;
                }
                case "0": {
                    List<Supplier> list = sdb.getListSupplier(0);
                    request.setAttribute("listSupplier", list);
                    request.setAttribute("status", status);
                    break;
                }
